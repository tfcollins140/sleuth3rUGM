...
/* VerD 9/6/2006 D. Donato */
  MPI_Bcast (&aux_diffusion_coeff,    sizeof (float), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast (&aux_diffusion_mult,     sizeof (float), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast (&aux_breed_coeff,        sizeof (float), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast (&WriteSlopeFileFlag,     sizeof (int),   MPI_INT,   0, MPI_COMM_WORLD);
  MPI_Bcast (&WriteRatioFileFlag,     sizeof (int),   MPI_INT,   0, MPI_COMM_WORLD);
  MPI_Bcast (&WriteXypointsFileFlag,  sizeof (int),   MPI_INT,   0, MPI_COMM_WORLD);
/* VerD 9/6/2006 D. Donato */
#endif
  ...
  while (fgets (line, SCEN_MAX_FILENAME_LEN, fp) != NULL)
  {
    ...
    if (strncmp (line, "#", 1))
    {
      ...
      if (strlen (line) > 0)
      {
        ...
        else if (!strcmp (keyword, "AUX_DIFFUSION_COEFF"))
        {
          object_ptr = strtok (NULL, "\0");
          util_trim (object_ptr);
		  aux_diffusion_coeff = atof (object_ptr);
        }
        else if (!strcmp (keyword, "AUX_BREED_COEFF"))
        {
          object_ptr = strtok (NULL, "\0");
          util_trim (object_ptr);
		  aux_breed_coeff = atof (object_ptr);
        }
        else if (!strcmp (keyword, "AUX_DIFFUSION_MULT"))
        {
          object_ptr = strtok (NULL, "\0");
          util_trim (object_ptr);
		  aux_diffusion_mult = atof (object_ptr);
        }

        else if (!strcmp (keyword, "WRITE_SLOPE_FILE(YES/NO)"))
        {
          object_ptr = strtok (NULL, " \n");
          util_trim (object_ptr);
          util_AllCAPS (object_ptr);
          WriteSlopeFileFlag = 1;
          if (!strcmp (object_ptr, "NO"))
          {
            WriteSlopeFileFlag = 0;
          }
        }
        else if (!strcmp (keyword, "WRITE_RATIO_FILE(YES/NO)"))
        {
          object_ptr = strtok (NULL, " \n");
          util_trim (object_ptr);
          util_AllCAPS (object_ptr);
          WriteRatioFileFlag = 1;
          if (!strcmp (object_ptr, "NO"))
          {
            WriteRatioFileFlag = 0;
          }
        }
        else if (!strcmp (keyword, "WRITE_XYPOINTS_FILE(YES/NO)"))
        {
          object_ptr = strtok (NULL, " \n");
          util_trim (object_ptr);
          util_AllCAPS (object_ptr);
          WriteXypointsFileFlag = 1;
          if (!strcmp (object_ptr, "NO"))
          {
            WriteXypointsFileFlag = 0;
          }
        }
  ...
  scen_echo (FILE * fp)
{
  ...
  fprintf (fp, "AUX_DIFFUSION_COEFF = %f\n", aux_diffusion_coeff);
  fprintf (fp, "AUX_BREED_COEFF = %f\n", aux_breed_coeff);
  fprintf (fp, "AUX_DIFFUSION_MULT = %f\n", aux_diffusion_mult);

  fprintf (fp, "WRITE_SLOPE_FILE = %u\n", WriteSlopeFileFlag);
  fprintf (fp, "WRITE_RATIO_FILE = %u\n", WriteRatioFileFlag);
  fprintf (fp, "WRITE_XYPOINTS_FILE = %u\n", WriteXypointsFileFlag);
  /*VerD*/
